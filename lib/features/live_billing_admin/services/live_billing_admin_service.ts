...
import {
  ...
  liveSessionItemQuantity,
  type BarcodeProductRef,
} from "@/lib/stock"
...
export const ADMIN_SCAN_SESSION_STORAGE_KEY =
  "samrat_admin_scan_session_id"
...
async function resolveUniqueBillNo(preferred: string): Promise<string> {
  if (!(await billNoAlreadyExists(preferred))) {
    return preferred
  }
  ...
  for (let n = 2; n <= 9; n++) {
    ...
    if (!(await billNoAlreadyExists(candidate))) {
      return candidate
    }
  }
...
async function deductStockForLineItem(
  itemPayload: LiveBillingLineItem
): Promise<void> {
  try {
    ...
    let productDoc = productSnap.empty
      ? null
      : productSnap.docs[0]
    ...
    if (!productDoc) {
      ...
      if (directSnap.exists()) {
        productDoc =
          directSnap as typeof productSnap.docs[0]
      }
    }
    ...
    const batchesList = batchesSnap.docs
      .map((d) => {
        ...
        if (
          bd.expiryDate &&
          typeof (bd.expiryDate as { toDate?: () => Date }).toDate ===
            "function"
        ) {
          expiryDate = (
            bd.expiryDate as { toDate: () => Date }
          ).toDate()
    ...
    const currentStock = firestoreNumber(
      productData.stock,
      0
    )
    ...
    const newStock = Math.max(
      0,
      currentStock - itemPayload.quantity
    )
    ...
    console.error(
      `Failed to deduct stock for barcode ${itemPayload.barcode}:`,
      stockErr
    )
  }
}
...
/**
 * Write a sales invoice and deduct stock.
 * Used by live complete and offline sync.
 */
export async function writeSaleFromLineItems(input: {
  ...
  const resolvedCustomer =
    await ensureCustomerForBilling(input.customer)

  const customerPhone =
    resolvedCustomer?.customerPhone?.trim() ||
    input.customer?.customerPhone?.trim() ||
    "NA"
  ...
  if (input.lineItems.length > 0) {
    billNo = await resolveUniqueBillNo(
      input.billNo ||
        generateOnlineBillNo(input.sessionId)
    )
...
/**
 * Reuse one active scanner session per browser tab.
 */
...
async function ensureCustomerForBilling(
  ...
  const phone = normalizeBillingPhone(
 ...
 * Reuse one active scanner session per browser tab.
 */
export async function getOrCreateScannerBillingSession(
  cashierLabel?: string
): Promise<string> {
  if (typeof window !== "undefined") {
  ...
  if (!scannerSessionCreatePromise) {
    scannerSessionCreatePromise =
      createScannerBillingSession(
        cashierLabel
      ).finally(() => {
...
/**
 * Start a fresh scanner session.
 */
export async function forceNewScannerBillingSession(
  cashierLabel?: string
): Promise<string> {
  clearAdminScanSessionStorage()

  return createScannerBillingSession(
    cashierLabel
  )
}

/**
 * Complete a live session by writing one consolidated
 * invoice into sales and updating live session status.
 */
export async function completeLiveBillingSession(
  ...
  const liveSnap = await getDoc(
    liveSessionRef
  )

  if (!liveSnap.exists()) {
    throw new Error(
      `Live session not found: ${sessionId}`
    )
  }
  ...
  /*
   * Idempotency:
   * If a sale was already created for this session,
   * don't create another sale.
   */
  ...
  if (existingSaleId) {
    ...
    if (
      liveData.globalStatsApplied !== true &&
      savedTotal > 0
    ) {
      try {
        await applyGlobalDashboardStatsIncrement(
          sessionId,
          savedTotal
        )
      } catch (aggregateErr) {
        console.error(
          "[DashboardAggregate] Bill already saved; global stats increment retry failed — reconcile later:",
          aggregateErr
        )
      }
    }
  ...
  /*
   * Required read for completing the active bill.
   * This is the live session's item collection.
   */
  ...
  for (const itemDoc of itemsSnap.docs) {
    ...
    const barcode = String(
      itemData.barcode ?? itemDoc.id
    )

    const basePrice = firestoreNumber(
      itemData.price,
      0
    )
    ...
    lineItems.push({
      ...
      price: basePrice,
      quantity:
        liveSessionItemQuantity(itemData),
      discountPercent,
      ...(mrpRaw > 0
        ? {
            mrp: mrpRaw,
  ...
  /*
   * Save sale ID on the live session so retries
  ...
  try {
    ...
    console.error(
      "Sale written but session status/update failed:",
      sessionErr
    )
  }

  /*
   * Update dashboard aggregate using the exact same
   * total that was written to the sales document.
   *
   * The aggregate service has session-level idempotency.
   */
  if (result.saleId && result.salesWritten > 0) {
    try {
      const saleSnap = await getDoc(
        doc(
          db,
      ...
      console.error(
        "[DashboardAggregate] Bill saved but global stats increment failed — reconcile later:",
        aggregateErr
      )
    }
  }
...
/**
 * Cancel a live session without writing anything
 * to completed_bills.
 */
export async function cancelLiveBillingSession(
  sessionId: string
): Promise<void> {
  const liveSessionRef = doc(
    db,
 ...
 */
export async function createScannerBillingSession(
  cashierLabel?: string
): Promise<string> {
  const ref = await addDoc(
    collection(db, col("live_sessions")),
    {
      status: "active",
      createdAt: Timestamp.now(),
...
export async function lookupProductForBilling(
  ...
  const scanned =
    normalizeScannedBarcode(barcode)

  if (!scanned) return null

  /*
   * Fastest path when the billing page already
   * has cached product data.
   */
  ...
  /*
   * Fast direct document-id lookup.
   */
  ...
  /*
   * Targeted barcode query.
   *
   * This replaces any full products collection scan.
   */
...
/**
 * Find existing session line item doc for a barcode.
 *
 * Fast path:
 *   Direct document lookup using barcode-derived ID.
 *
 * Slow path:
 *   Full session item scan only for legacy/mobile
 *   sessions with non-standard document IDs.
 */
async function findSessionItemRef(
  ...
  for (const raw of barcodes) {
    for (const c of getBarcodeLookupCandidates(raw)) {
      candidates.add(c)
    }
  }
  ...
  /*
   * Fast path:
   * Admin scanner uses document ID derived from barcode.
   */
   ...
   * Legacy/mobile sessions may use non-standard
   * item document IDs.
   */
  ...
  for (const itemDoc of itemsSnap.docs) {
    ...
    for (const raw of barcodesToCheck) {
      ...
      if (
        getBarcodeLookupCandidates(raw).some(
          (c) => candidates.has(c)
        )
      ) {
        return itemDoc.ref
...
export async function scanItemIntoSession(
  ...
  if (!product) {
    throw new Error(
      `Product not found for barcode: ${
        cleaned || barcode.trim()
      }`
    )
  }

  const existingRef =
    await findSessionItemRef(
      sessionId,
      product.barcode,
      cleaned
    )

  const billedDiscount =
    clampDiscountPercent(
  ...
  if (existingRef) {
    ...
    const data =
      (updatedSnap.data() ?? {}) as Record<
        string,
        unknown
      >

    const qty =
      liveSessionItemQuantity(data)
...
export function toSessionItemDocId(
  barcode: string
): string {
...
function newManualItemDocId(): string {
  return `manual_${Date.now().toString(
    36
  )}_${Math.random()
    .toString(36)
    .slice(2, 8)}`.replace(
...
/**
 * Add a custom line item (not from inventory scan).
 */
...
/**
 * Set line quantity manually.
 * Removes line if qty <= 0.
 */
...
/**
 * Override selling rate for a line item.
 */
